...
public class ManagerCheckFilter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		 ...
		 * key.equals("elec")) { // elec 접근 HttpServletRequest req =
		 * (HttpServletRequest)request; HttpSession session = req.getSession();
		 * if(session.getAttribute("loginUser") == null) { // 인증되지 않은 사용자
